...
export default function LRList({
  ...
  const onSendEmailHandler = async () => {
    ...
    emailIds.split(",").forEach(async (email: string) => {
      if (selectedLR && email !== undefined && email !== null && email.trim() !== "") {
        selectedLR.mailBody = mailGreeting;
        const formData = new FormData();
        formData.append("file", attachment, "LorryReceipt.pdf");
        formData.append("LrData", JSON.stringify(selectedLR));
          ...
          <tbody>
            {LRData.map((data) => (
              ...
              >
                ...
                <td className="py-2">{data.consigneeName}</td>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex justify-end gap-5">
                      ...
                      <Button
                        ...
                        disabled={isLoading}
                      >
                        {isLoading ? <VscLoading size={20} className="animate-spin" /> : "Send Mail"}
                      </Button>
                    </div>
                  </div>
                </div>
